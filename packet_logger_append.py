#!/usr/bin/env python3
"""
nabiot_capture_append.py

Capture live IoT traffic, compute N-BaIoT’s features, and continuously append
feature vectors to a CSV, calculating covariance and Pearson correlation correctly.
"""
...
# --- Optional: If you have SciPy installed uncomment below: ---
...
# --- Initialize Buffers -----------------------------------------------------
buffers = { w: { 'H': deque(), 'MI': deque(), 'HH': deque(), 'HH_jit': deque(), 'HpHp': deque() } for w in WINDOWS }
...
def dir_stats(dq):
    ...
    cnt = len(ins) + len(outs)
    mean = statistics.mean(ins+outs) if cnt>0 else 0.0
    var = statistics.pvariance(ins+outs) if cnt>1 else 0.0
    std = math.sqrt(var)
    return cnt, mean, var, std
...
def jitter_stats(dq):
    ...
    var = statistics.pvariance(diffs) if cnt>1 else 0.0
    return cnt, mean, var
...
def combined_metrics(dq):
    ...
    try:
        cov = statistics.pcovariance(in_s, out_s) if len(in_s)>1 and len(out_s)>1 else 0.0
        # If using SciPy:
        # corr = pearsonr(in_s, out_s)[0] if len(in_s)>1 and len(out_s)>1 else 0.0
        # Manual Pearson r:
        denom = math.sqrt(statistics.pvariance(in_s) * statistics.pvariance(out_s)) if len(in_s)>1 and len(out_s)>1 else 0.0
        corr = cov/denom if denom>0 else 0.0
    except Exception:
        cov = corr = 0.0
    return mag, rad, cov, corr

# --- Packet Processing & CSV Append -----------------------------------------
def process_packet(pkt):
    ...
    for lbl, w in zip(LABELS, WINDOWS):
        cnt, mean, var, _ = dir_stats(buffers[w]['MI'])
        row += [cnt, mean, var]
    for lbl, w in zip(LABELS, WINDOWS):
        cnt, mean, var, _ = dir_stats(buffers[w]['H'])
        row += [cnt, mean, var]
    for lbl, w in zip(LABELS, WINDOWS):
        cnt, mean, var, std = stats_stream(buffers[w]['HH'])
        mag, rad, cov, corr = combined_metrics(buffers[w]['HH'])
        row += [cnt, std, std, mag, rad, cov, corr]
    for lbl, w in zip(LABELS, WINDOWS):
        cnt, mean, var = jitter_stats(buffers[w]['HH_jit'])
        row += [cnt, mean, var]
    for lbl, w in zip(LABELS, WINDOWS):
        cnt, mean, var, std = stats_stream(buffers[w]['HpHp'])
        mag, rad, cov, corr = combined_metrics(buffers[w]['HpHp'])
...
if __name__ == "__main__":
    print(f"Starting capture on {INTERFACE}, appending to {OUTPUT_CSV}")
    # Write header if new
    try:
